...
public class JMSSender {
	...
	public static void main(String[] args) throws NamingException, JMSException {
		...
		QueueConnectionFactory connFactory = (QueueConnectionFactory) ctx.lookup(Config.getValue("CONNECTION_FACTORY_NAME"));
		QueueConnection qConn = (QueueConnection) connFactory.createConnection();
		QueueSession qSession = qConn.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
		Queue queue = (Queue) ctx.lookup(Config.getValue("QUEUE_NAME"));
